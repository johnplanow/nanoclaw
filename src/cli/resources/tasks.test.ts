...
describe('tasks CLI resource', () => {
  ...
  // Fork (docs/CUSTOMIZATIONS.md §10): --in-origin-session / --session run a task INSIDE a chat session.
  describe('tasks inside a chat session (--in-origin-session)', () => {
    it('agent --in-origin-session writes the task into the caller chat session and it stays listable', async () => {
      ...
      expect((await getSessionsByAgentGroup('ag-1')).some((s) => s.thread_id === taskThreadId(created.series_id))).toBe(
        false,
      );
